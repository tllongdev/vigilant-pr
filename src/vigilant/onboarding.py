...
def run_init(env_path: str = ".env") -> int:
    ...
    print("Vigilant PR setup - this writes a local .env you can edit anytime.\n")

    # 1) GitHub access (reviews post as you).
    gh_problem = github_preflight()
    if gh_problem:
        print("GitHub access is not ready yet:\n")
        print("  " + gh_problem.replace("\n", "\n  "))
        print("\nFix that (usually `gh auth login`), then re-run `vigilant init`.")
        if not _yes("Continue setup anyway?", default=False):
            return 1
    else:
    ...
    if pc is None:
        ...
        model = _prompt("Ollama model", "ollama/qwen2.5:14b")
        updates["VIGILANT_MODEL"] = model
        ...
        if not key:
            ...
            if _verify_key(pc.key):
                print("  Key works.")
            else:
                print("  Could not auto-verify (that's OK - saving it anyway).")
        updates["VIGILANT_MODEL"] = pc.model
        print(f"Default model set to {pc.model} (override with --opus/--sonnet or --model).")

    # 3) Optional Slack monitoring.
    ...
    if not updates:
        print("\nNothing to write. Re-run when you're ready.")
        return 0
